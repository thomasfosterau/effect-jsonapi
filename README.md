...
Document.DataDocument(Document.nullable(Article)) //       data: Option<Article>, ⇆ null on the wire
```

`Document.nullable` is `Schema.OptionFromNullOr` — the spec-clean nullable
codec (`None ⇆ null`). Avoid effect's _structural_ `Schema.Option`
...
| `null` primary `data` only for a single-resource URL that might correspond to a resource but currently doesn't   | `DataDocument` is a pure envelope: `DataDocument(R)` is non-null; opt into `data: null` with `Schema.NullOr(R)` / `Document.nullable(R)`. `Endpoint.related` keeps the nullable form for a to-one relationship |
