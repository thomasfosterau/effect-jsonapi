...
DataDocument(Document.nullable(Article)) //   data: Option<Article>, ⇆ null on the wire
```
...
- New `Document.nullable` — the blessed, spec-clean nullable codec (a re-export of `Schema.OptionFromNullOr`, `None ⇆ null`). Prefer it over effect's structural `Schema.Option` (`{ _tag, value }`), which would serialise a non-conformant body.
